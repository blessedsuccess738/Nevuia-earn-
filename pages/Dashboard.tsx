...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  return (
    <div className="flex-grow pb-32 px-4 pt-6 max-w-md mx-auto w-full space-y-6 animate-in fade-in duration-500">
      ...
      <div className="glass-card p-6 rounded-[2.5rem] border border-white/10 relative overflow-hidden shadow-2xl">
        <div className="relative z-10 flex items-center gap-4 mb-4">
           <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-green-500 border border-white/10 shadow-inner">
        ...
        <div className="relative z-10 grid grid-cols-1 gap-2 border-t border-white/5 pt-4">
           <div className="flex justify-between items-center">
              <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Available Balance</p>
              <h2 className="text-2xl font-black text-white tracking-tighter">${user.balanceUSD.toFixed(2)}</h2>
           </div>
           <div className="flex justify-between items-center">
              <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Local Valuation</p>
              <h2 className="text-lg font-black text-green-500 tracking-tighter">₦{(user.balanceUSD * settings.usdToNgnRate).toLocaleString()}</h2>
           </div>
        </div>
      </div>

      {/* Main Menu Grid - Mobile Style */}
      <div className="grid grid-cols-4 gap-3">
        {menuItems.map((item) => (
          ...
          >
            <div className={`w-14 h-14 ${item.color} rounded-2xl flex items-center justify-center text-white text-xl shadow-lg active:scale-90 transition-all`}>
              <i className={item.icon}></i>
            </div>
            <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 group-hover:text-white">{item.label}</span>
          </button>
        ))}
      </div>
      ...
      <button 
        ...
        className="w-full bg-blue-600 p-4 rounded-3xl flex items-center justify-between shadow-2xl active:scale-95 transition-all group"
      >
        <div className="flex items-center gap-3">
           ...
           <div className="text-left">
              <h3 className="text-white font-black uppercase italic tracking-tighter text-sm">Global Chat</h3>
              <p className="text-[9px] text-white/60 font-black uppercase tracking-widest">{onlineUsers} Earner Nodes Active</p>
           </div>
        </div>
        <i className="fas fa-chevron-right text-white/50 group-hover:text-white transition-all text-sm"></i>
      </button>

      {/* Platform Grid */}
      <div className="space-y-3">
         <h3 className="text-[10px] text-gray-500 font-black uppercase tracking-[0.4em] ml-2">Mining Hubs</h3>
         <div className="grid grid-cols-2 gap-3">
            {platforms.map(p => (
              <button 
                key={p.name}
                onClick={() => navigate(`/listen?cat=${p.cat}`)}
                className="glass-card p-5 rounded-[2rem] border border-white/5 flex flex-col items-center gap-3 hover:bg-white/5 transition-all active:scale-95 shadow-lg"
              >
                {p.img ? (
                  <img src={p.img} className="w-8 h-8 object-contain" alt={p.name} />
                ) : (
      ...
      <div className="glass-card p-5 rounded-[2rem] border border-white/10 flex items-center justify-between bg-black/40">
         <div className="flex items-center gap-3">
            ...
            <div>
               <p className="text-[8px] text-gray-500 font-black uppercase tracking-widest mb-0.5">Referral Protocol</p>
               <h4 className="text-white font-black text-xs uppercase italic tracking-tighter">{user.referralCode}</h4>
            </div>
         </div>
         <button onClick={handleCopyRef} className="bg-white text-black px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-green-500 transition-all">Copy</button>
      </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-2xl animate-in fade-in duration-300 flex flex-col">
          <div className="p-6 border-b border-white/10 flex items-center justify-between bg-black/40">
            <div className="flex items-center gap-3">
               ...
               <div>
                  <h3 className="text-white font-black uppercase italic tracking-tight text-sm">Global Stream</h3>
            ...
            {publicChat.map(m => (
              <div key={m.id} className="animate-in slide-in-from-bottom-2">
                <div className="flex items-center gap-2 mb-1">
                   <span className={`text-[7px] font-black uppercase px-1.5 py-0.5 rounded ${m.tier === PlanTier.PREMIUM ? 'bg-yellow-500 text-black' : 'bg-white/10 text-gray-500'}`}>{m.tier}</span>
                   <span className="text-[9px] font-black text-white italic truncate max-w-[100px]">@{m.username}</span>
                </div>
                <div className="bg-white/5 p-4 rounded-2xl rounded-tl-none border border-white/5 text-[11px] text-gray-300 font-medium leading-relaxed">
                  {m.text}
                </div>
              </div>
            ))}
