...
const ListenEarn: React.FC<ListenEarnProps> = ({ onReward, settings, user, tracks }) => {
  ...
  const togglePlay = (track: MusicTrack) => {
    if (user.playedTracksToday.includes(track.id)) { alert("Royalty already collected for this node."); return; }
    if (limitReached) { alert("Daily listening limit reached."); return; }
  ...
  return (
    <div className="flex-grow pb-40 flex flex-col items-center animate-in fade-in duration-300">
      
      {/* Platform Playback Board */}
      {activeTrack && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 w-[92%] max-w-sm z-[80] bg-black/80 backdrop-blur-3xl border border-white/10 rounded-[2rem] p-4 shadow-2xl animate-in slide-in-from-top-4 duration-500">
          <div className="flex items-center gap-4">
             <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center border border-white/10">
                <img src={CATEGORY_ICONS[activeTrack.category] || SPOTIFY_PNG} className="w-10 h-10 object-contain animate-pulse" alt="Logo" />
             </div>
             <div className="flex-grow min-w-0">
                <div className="flex items-center gap-2 mb-1">
                   <span className="text-[8px] font-black uppercase text-green-500 tracking-[0.2em]">{activeTrack.category} NETWORK</span>
                </div>
                <h2 className="text-white font-black text-xs uppercase italic truncate">{activeTrack.title}</h2>
      ...
      <div className={`w-full max-w-md p-6 transition-all ${activeTrack ? 'pt-40' : 'pt-6'}`}>
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <p className="text-[8px] text-gray-500 font-black uppercase">Next Protocol Reset</p>
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-green-500 rounded-xl flex items-center justify-center shadow-lg shadow-green-500/20">
               <i className="fas fa-music text-black text-sm"></i>
            </div>
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search infrastructure..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-[1.5rem] py-4 pl-12 pr-6 text-white text-sm font-bold focus:border-green-500 transition-all shadow-xl" />
          </div>
            ...
            {filteredTracks.map(track => {
               ...
               return (
                <div 
                  ...
                  className={`glass-card p-4 rounded-[1.8rem] border flex items-center gap-4 transition-all ${
                    activeTrackId === track.id ? 'border-green-500 bg-green-500/10' : 'border-white/5'
                  } ${isPlayed ? 'opacity-40 grayscale' : 'cursor-pointer hover:bg-white/5 active:scale-[0.98]'}`}
                >
                  <div className="relative">
                    <div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center border border-white/5">
                      <img src={CATEGORY_ICONS[track.category] || SPOTIFY_PNG} className="w-10 h-10 object-contain" alt="Platform" />
                    </div>
                    {isPlayed && <div className="absolute inset-0 bg-black/60 rounded-2xl flex items-center justify-center text-green-500"><i className="fas fa-check-circle text-xl"></i></div>}
                  </div>
                  <div className="flex-grow min-w-0">
                    <h3 className="text-white font-black text-xs uppercase italic truncate mb-1">{track.title}</h3>
                    <div className="flex items-center gap-2">
                       <span className="text-green-500 text-[10px] font-black tracking-tight">+${track.earningUSD.toFixed(2)}</span>
                       <span className="text-[8px] text-gray-600 font-bold uppercase tracking-widest">{track.duration}S</span>
                    </div>
                  </div>
                  <div className={`w-10 h-10 rounded-full border border-white/10 flex items-center justify-center ${activeTrackId === track.id && isPlaying ? 'bg-green-500 text-black border-transparent shadow-lg' : 'text-gray-500'}`}>
                     <i className={`fas ${activeTrackId === track.id && isPlaying ? 'fa-volume-up animate-bounce' : 'fa-play'} text-[10px]`}></i>
                  </div>
                </div>
               );
            })}
      ...
      {showRewardModal && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-6 backdrop-blur-xl">
           <div className="glass-card max-w-sm w-full p-10 rounded-[3rem] text-center border border-green-500/30">
              <div className="w-20 h-20 bg-green-500 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-2xl border-4 border-black rotate-3">
                 <i className="fas fa-sack-dollar text-black text-3xl"></i>
              </div>
              <h2 className="text-2xl font-black uppercase italic mb-2 tracking-tighter text-white leading-none">Yield Captured!</h2>
              <p className="text-[10px] text-gray-500 font-black uppercase mb-10 tracking-[0.2em] leading-relaxed">Asset liquidity injected: <span className="text-green-500">+${showRewardModal.amount.toFixed(2)}</span></p>
              <button onClick={() => setShowRewardModal(null)} className="w-full bg-white text-black py-5 rounded-[1.5rem] font-black uppercase tracking-[0.3em] text-[10px] shadow-2xl hover:bg-green-500 transition-all active:scale-95">Collect Royality</button>
           </div>
        </div>
      )}
    </div>
